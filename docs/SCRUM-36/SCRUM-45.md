...
Functional tests will be implemented with concrete implementations in other stories. This story focuses on interface definitions only.

---

## Edge Cases

1. **Interface Compliance:** Ensure all implementations strictly follow interface contracts
2. **Type Safety:** Use PHP 8.1+ type hints for all parameters and return types
3. **Nullability:** Use nullable types (`?Type`) where appropriate
4. **Documentation:** PHPDoc must include @throws annotations for exceptions
